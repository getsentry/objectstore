...
//!
//! Enable the `resumable-upload-api` feature to use this manual API. It maps directly to the server
//! operations and is intended for advanced use cases that need explicit resumable-upload control.
...
impl Session {
    ...
    ///
    /// `object_length` is the length of the bytes supplied to [`ResumableUpload::put_chunk`],
    /// after any compression performed by the caller.
    ///
...
impl ResumableUpload {
    ...
    ///
    /// The client sends `body` verbatim. When compression is configured on the upload, the caller
    /// must compress each chunk before passing it to this method.
    ///
...
impl CreateResumableUploadBuilder {
    ...
    ///
    /// Unlike regular uploads, the client does not automatically compress chunk contents. The
    /// caller must compress them according to `compression`. The object length and offsets refer
    /// to the bytes after compression.
    ///
    /// By default, the compression algorithm set on this session's use case is used.
