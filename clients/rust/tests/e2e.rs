...
#[tokio::test]
async fn filename_with_quotes_roundtrips() {
    let server = test_server().await;
    ...
    let client = Client::builder(server.url("/"))
        ...
        .unwrap();
    let session = client
        .session(Usecase::new("usecase").for_organization(12345))
        .unwrap();

    let stored_id = session
        .put("data")
        .compression(None)
        .filename(r#"has"quote.txt"#)
        .key("fn-quotes")
        .send()
        .await
        .unwrap()
    ...
    let response = session.get(&stored_id).send().await.unwrap().unwrap();
    assert_eq!(
        response.metadata.filename.as_deref(),
        Some("has\"quote.txt")
    );
}

#[tokio::test]
async fn filename_with_slashes_roundtrips() {
    let server = test_server().await;
    ...
    // The raw filename is preserved in metadata
    let response = session.get(&stored_id).send().await.unwrap().unwrap();
    assert_eq!(
        response.metadata.filename.as_deref(),
...
#[tokio::test]
async fn filename_dots_only_roundtrips() {
    let server = test_server().await;
    ...
    for (key, name) in [("fn-dot", "."), ("fn-dotdot", "..")] {
        let stored_id = session
            .put("data")
            .compression(None)
            .filename(name)
        ...
        // The raw filename is preserved in metadata
        let response = session.get(&stored_id).send().await.unwrap().unwrap();
        assert_eq!(response.metadata.filename.as_deref(), Some(name));
    }
}
