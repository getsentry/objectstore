...
///   `override_with()`
/// - Module-scoped `OnceLock` statics for the singleton snapshot (`ArcSwap<T>`) and the raw
///   sentry-options handle that the live-reload callback reads from
...
fn expand(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    Ok(quote! {
        ...
        static __INNER: ::std::sync::OnceLock<
        ...
        impl #name {
            ...
            pub fn init() -> ::std::result::Result<(), ::objectstore_typed_options::Error> {
                ...
                if __OPTIONS.get().is_some() {
                    return ::std::result::Result::Err(
                        ::objectstore_typed_options::sentry_options::OptionsError::AlreadyInitialized.into(),
                    );
                }

                // Load an initial snapshot and fail loudly if it can't be loaded. This ensures the
                // application will not silently run with defaults or fail later when options are
                // accessed.
                let inner = ::objectstore_typed_options::sentry_options::Options::builder()
                    .with_schemas(&[(Self::NAMESPACE, Self::SCHEMA)])
