...
//! singleton, schema validation at startup, atomic background refresh every few seconds,
...
//!   spawns a background refresh task. Idempotent. Must be called inside a Tokio runtime.
//! - `T::override_with(…)` — *(testing feature only)* Temporarily overrides option values;
//!   reverts when the returned guard is dropped.
//!
//! The background refresh task reloads values from disk every 5 seconds and atomically swaps
//! in the new snapshot without blocking readers.
//!
//! # Testing
//!
...
//! }
//!
//! // At startup (inside a Tokio runtime):
...
#[doc(hidden)]
pub use {arc_swap, sentry_options, serde, serde_json, tokio};
...
///
/// Called by the generated [`init`](SentryOptions) implementation. Not intended for direct use.
pub async fn refresh<T: SentryOptions>(
    lock: &'static OnceLock<ArcSwap<T>>,
    inner: sentry_options::Options,
) {
    let Some(snapshot) = lock.get() else {
        return;
    };
