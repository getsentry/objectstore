...
impl Operation {
    async fn try_from_field(field: Field<'_>) -> Result<Self, BatchError> {
        ...
        let key = ObjectKey::from_encoded(key_header).map_err(|e| {
            BatchError::BadRequest(format!(
                "invalid {HEADER_BATCH_OPERATION_KEY} header value: {e}"
            ))
        })?;
...
mod tests {
    ...
    async fn test_valid_request_works() {
        ...
        assert_eq!(get_op.key, ObjectKey::from_encoded_unchecked("test0"));

        let Operation::Insert(insert_op1) = &operations[1].as_ref().unwrap() else {
            panic!("expected insert operation");
        };
        assert_eq!(insert_op1.key, ObjectKey::from_encoded_unchecked("test1"));
        ...
        assert_eq!(insert_op2.key, ObjectKey::from_encoded_unchecked("test2"));
        ...
        assert_eq!(delete_op.key, ObjectKey::from_encoded_unchecked("test3"));
    }
