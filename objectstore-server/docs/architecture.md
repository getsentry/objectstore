...
2. **Extractors**: path parameters are parsed into an
   ...
   read from the `X-Objectstore-Auth` header (preferred), the
   `x-objectstore-auth` query parameter, or the standard `Authorization`
   header (fallback), then validated and decoded into an
