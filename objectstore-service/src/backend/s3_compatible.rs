...
mod tests {
    ...
    fn create_test_backend() -> S3Compatible<NoToken> {
        S3Compatible::without_token("http://localhost:8087", "test-bucket")
    }

    #[tokio::test]
    async fn test_s3_compatible() {
        let backend = S3Compatible::without_token("http://localhost:8087", "test-bucket");
    }
