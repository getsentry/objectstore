...
use crate::error::{BackendDetail, BackendError, Error, Result};
...
/// [`Error::Backend`] with the extracted error code and message.
///
...
pub trait ResponseExt {
    ...
    /// wrapped as [`Error::Backend`] with the same context string.
...
impl ResponseExt for Response {
    async fn check_error(self, context: &'static str) -> Result<Response> {
        ...
        Err(BackendError::from_status(context, status, detail).into())
    }
