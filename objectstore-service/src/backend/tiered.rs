...
/// Amount of time for which a `Change` generated by a `complete_multipart` operation is kept in the `Assembling`
/// state before becoming eligible for cleanup by the `ChangeLog` recovery process.
/// This allows the client to retry the `complete_multipart` operation upon any failures for at least this long,
/// avoiding scenarios where the `ChangeLog` recovery would race to delete the assembled LT blob.
const MULTIPART_COMPLETE_CLEANUP_DELAY: Duration = Duration::from_hours(24);
...
impl TieredStorage {
    ...
    /// Records the change to the log in the `Assembling` phase, and returns a guard that does
    /// nothing on drop unless advanced.
...
impl TryInto<UploadId> for TieredUploadId {
    ...
    fn try_into(self) -> Result<UploadId, Self::Error> {
        let json = serde_json::to_vec(&self)
            .map_err(|e| Error::bad_request("encoding multipart token", e))?;
...
impl TryFrom<&UploadId> for TieredUploadId {
    ...
    fn try_from(value: &UploadId) -> Result<Self, Self::Error> {
        let json = base64::engine::general_purpose::URL_SAFE_NO_PAD
            ...
            .map_err(|e| Error::bad_request_msg(format!("invalid multipart upload ID: {e}")))?;
        serde_json::from_slice(&json).map_err(|e| Error::bad_request("decoding multipart token", e))
    }
}
    ...
    async fn complete_multipart(
        ...
        let mut guard = self
            .record_assembling(Change {
                ...
                old: current.clone(),
                cleanup_after: Some(SystemTime::now() + MULTIPART_COMPLETE_CLEANUP_DELAY),
            })
            .await?;
...
mod tests {
    ...
    async fn multipart_overwrites_existing_tombstone() {
        ...
        // The multipart upload is not finalized, so the tombstone still points to the old
        // revision.
        let lt_id = hv.get(&id).expect_tombstone().target;
        assert_eq!(old_lt_id, lt_id);
        ...
        // Now the upload has been finalized, so the new tombstone points to the new revision.
        ...
        // Old revision should be cleaned up.
    ...
    async fn cleans_up_orphan_after_failed_multipart_complete() {
        ...
        // The LT blob is orphaned, and no cleanup has been performed (yet), due to the guard being
        // dropped while in the `Assembling` state.
        ...
        // The orphaned LT blob has been cleaned up.
    ...
    #[derive(Debug)]
    struct FailOnFirstCompleteMultipartAttempt {
        attempt: Mutex<u32>,
    }

    impl FailOnFirstCompleteMultipartAttempt {
        fn new() -> Self {
            Self {
                attempt: Mutex::new(0),
            }
    ...
    #[async_trait::async_trait]
    impl Hooks for FailOnFirstCompleteMultipartAttempt {
        async fn complete_multipart(
            &self,
            inner: &InMemoryBackend,
            id: &ObjectId,
    ...
    async fn multipart_complete_succeeds_on_retry_and_leaves_state_consistent() {
        ...
        let storage = TieredStorage::new(
            ...
            Box::new(TestBackend::with_inner(
                lt_inner.clone(),
                FailOnFirstCompleteMultipartAttempt::new(),
            )),
            Box::new(log.clone()),
        );
